...
Stack Exploitation | Overview
The stack is vulnerable to a variety of exploits, the most fundamental of which is the simple stack-based buffer overflow. Understanding this type of overflow first requires an understanding of the stack and how programs and functions are placed into memory.

﻿

The simple stack-based buffer overflow forms the foundation upon which more advanced forms of stack exploitation, such as Return-Oriented Programming (ROP) and Structured Exception Handler (SEH) exploits, are built, as these attacks against stack vulnerabilities are essentially overflows with unique targets of attack.
...
When a program is executed, the various sections of the executable file from which a program is run are placed into memory in a specified format. These are the arguments and environment variables from the parent process context, the data (initialized and uninitialized), and the text section, which contains code.

![image](https://github.com/user-attachments/assets/65e52d85-a521-404b-bd6a-454be987e479)

The Stack


The stack is a type of data structure located in memory that is allocated by an OS for each individual process thread upon creation. A thread uses this structure for simple, fast memory operations that can be predicted at runtime. The stack is limited in size, defined in size on creations, and requires relatively simple but fast management operations. It operates on the Last In, First Out (LIFO) principle, which means that the data objects most recently placed, or pushed, on top of the stack are the first ones to be removed, or popped, from the stack. 


The stack grows from high-memory addresses to low-memory addresses. Thus, if the top of the stack points to the address 0xFFFFFFFF and four bytes of data are pushed onto the stack, the top of the stack then points to the address 0xFFFFFFFB.


The Function Call


When a function is called by the OS, that function gets its own slice of the thread's stack through a function call operation. In this procedure, the current state of the thread is saved by pushing to the stack the values of the current Extended Base Pointer (EBP) Register that points to the relative bottom of the stack, Extended Stack Pointer (ESP) Register that points to the top of the stack, and the address of the next operation to execute when the function returns. That value is stored in the Extended Instruction Pointer (EIP) Register. Then the values of those registers are overwritten to reflect the state of the new function. 


When the function returns, those saved values are retrieved from the stack and execution continues in the thread where it left off when the function was called. It is this principle of retrieving the address of the next instruction to execute that is exploited when a system allows that area of the stack to be overwritten.


Buffer Overflow


Simple buffer overflow occurs when a program writes more data to a fixed-length block of memory, known as a buffer, than that buffer is specified to hold. The overwrite overflows the boundaries of the buffer into adjacent areas of the stack, and it corrupts neighboring data. Attackers manipulate this reality by carefully crafting the data they send as input to a process, leading to control of execution flow and therefore arbitrary execution of an attacker's own code.


When a program handles this problem correctly, the input is truncated to prevent an overwrite. However, when such checking of bounds is not in place, the input overwrites the instruction pointer to which execution returns when the function ends.


Figure 7.4-2 provides a visualization of normal memory layout:

![image](https://github.com/user-attachments/assets/797393fe-feff-4c78-a685-7632cad70eca)

When an adversary executes the buffer overflow successfully, the layout is overwritten as Figure 7.4-3 illustrates, with the return address that is popped into the EIP on the function's return overwritten to obtain control of execution.

![image](https://github.com/user-attachments/assets/02fc14b0-556a-473a-9f54-4562022b38b6)
...
DEP is a hardware-based method of marking certain areas of memory (e.g., stack, heap, memory pool structures) as non-executable and treating attempts to execute code in those areas as an error. 


Other Stack Protections


Other techniques that mitigate threats to the stack include stack canaries, which defend against overflows in general, and exception handling protection, which attacks an entirely different data structure on the stack than the function call targeted in simple buffer overflows. 


Stack Canaries


Stack canaries are values placed between a function and the previous return address that are checked by a program's code to prevent a return to the parent process when the stack is overwritten.


Security Exception Handling Overwrite Protection (SEHOP)


SEHOP is the protection of the exception handler records for a function, protecting overflows into their return addresses leading to command execution.


Address Space Layout Randomization (ASLR)


ASLR is randomizing the locations of executable code in memory so that if attackers are able to bypass any of the previous mitigations, they cannot direct execution to a predictable address of memory marked as executable by the OS.


Each of these four types of mitigation is explored in further detail later in this lesson.
...
Memory Safeguards | DEP Overview
DEP is the memory safeguard that protects a system against the ability of adversaries to place their own malicious code in areas of memory that normally store or receive input, such as the stack or heap. 

﻿

By designating some sections of memory as executable only and other sections (such as the stack) as non-executable, an OS can achieve a measure of security. Then, even when an attacker overflows the stack to write over the instruction pointer, the system is prevented from executing shellcode that the attacker placed on the stack in the first place.

﻿

This form of stack execution prevention is essential in the modern computing age. Hardware-based DEP drastically reduces the attack surface area for memory-based attacks on applications by setting the No Execute (NX) bit in memory pages as a method of informing the system how memory is handled. When this is employed, memory is marked as read-only or execute-only, but not both. 

﻿

Areas typically marked as read-only are the stack, pages allocated by the heap, the text and data sections imported from a program image, and other memory pools. If an attacker manages to land code in one of those regions of memory, DEP throws an exception when the system tries to execute it.
...
1: AlwaysOn. DEP is enabled for all processes. Applications cannot explicitly disable DEP with this setting.
2: OptIn. DEP is enabled only for Windows system components and services, though processes may enable it.
3: OptOut. DEP is enabled for all processes. Administrators can manually create specific exceptions to this. This is the default configuration.
Bypassing DEP
﻿

Although highly effective, DEP is not a foolproof protection. This safeguard may be bypassed in one of the following ways:

ROP.
Marking non-executable memory as executable.
Turning off DEP for a process.
Turning off DEP for a system.
Notably, marking the stack as non-executable does not prevent it from being overwritten. That is the role of stack-smashing protections, discussed later.
...
The ROP bypass involves overwriting the return instruction pointer with an address of a piece of code in a known memory location that has already been marked as executable, such as instructions in the vulnerable binary or imported shared libraries. This overwrite, followed either by the arguments necessary for a desired system call or by a chain of other instruction addresses and returns, achieves, for the attacker, arbitrary code execution that effectively evades DEP, albeit at a significant cost in time and expertise. This is possible because although DEP prevents execution of code that is located in the stack or heap, the attacker can still abuse the code in areas marked as executable and simply pass a fake function call frame to them. 

﻿

One of the forms of this method is known as the return-to-libc attack. In this form, the return address written by the attacker is specifically located in the libc libraries in Linux or a standard Dynamic Link Library (DLL) in Windows. Often, this means finding an exec() or system() call and passing the path to a shell (such as cmd) to it. 

﻿

Marking Memory as Executable
﻿

If an attacker can direct execution to a virtual memory protection system call with the base address and size of the memory page in which an attacker has placed their code, then the protection that DEP affords is no longer present. These types of function calls, such as ZwProtectVirtualMemory or NTProtectVirtualMemory, tend to be located in kernel-level libraries like ntoskrnl.exe. 
...
Because with the various DEP settings the policy may need to be turned on or off on a per-process basis, an alternative bypass is to locate that system function and turn DEP off for the process being exploited. This system function has, at times, been named SetProcessDEPPolicy and NtSetInformationProcess. Each of these bypasses, which rely on the ability to jump to predictable addresses in shared or system libraries, are effectively defeated by ASLR.
...
Memory Safeguards | Other Stack Protections
Aside from DEP, developers have used code-based methods of detecting abnormal stack changes to prevent the classic stack smashing that occurs when a buffer overflows exploit is attempted. These methods employ the use of a stack canary.
...
Stack canaries, also known as stack cookies, are values placed between the return address and the rest of a function's stack with corresponding code that tests whether the stack has been smashed and the buffer overflowed. In this protection, when the system calls the return address, it first checks to see that the canary value matches the value placed in memory prior to the function call. When the value does not match, a security exception is triggered and the code does not execute. Instead, execution control is handed by the OS to a structured exception handler in Windows or exits with an error in Linux.
...
Because errors can occur in a variety of unknown situations, developers write code to handle those exceptions gracefully to avoid a crash. This sort of structured handling is primarily a function provided to developers by the compiler with OS support. SEH is employed to address a variety of hardware and software faults.
...
An SEH exploit overflows the address of the function handle that is called when an application-specific exception occurs and then triggers that exception. In this case, even overwriting the canary value is not a concern, because when the exception handler is called, that call executes malicious code before the canary value is even checked, as a return from the current function is not triggered. This is possible only when the attacker is able to both write enough data to overwrite that exception handler address and trigger the corresponding exception.
...
After SEH exploits became commonplace, Microsoft developed a protection called SafeSEH, which has been replaced in modern systems by SEHOP. This protection is provided at runtime and does not depend on any metadata from the binary it protects. However, it mainly prevents SEH overwrites by examining a thread's exception handler list for tampering before allowing any of its registered exception handlers to be called. This protects processes regardless of whether they were compiled with any SEH protections, such as SafeSEH.

﻿

SEHOP accomplishes these checks by examining the pointer records in the exception handler list. A side effect of overflowing a vulnerable buffer into the registered exception handler record is overwriting that record's pointer to the following record. This corruption is detected by the SEHOP procedure as the list is traversed prior to any exception handling execution. If the entire list of registered handlers is intact during this integrity check, the handler is allowed to execute.

﻿

The Group Policy setting Process Mitigation Options can be used to control the SEHOP setting. Some applications can experience problems with SEHOP, so they should be tested in an environment with this protection enabled.


---------------------

Memory Safeguards | ASLR
ASLR
﻿

ASLR causes a change in the memory addresses for shared libraries to make the location of executable addresses unreliable for stack overflow, especially such ROP exploits as return-to-libc. This protection loads executable code into random memory addresses to mitigate attacks that target specific memory locations as in the ROP DEP bypass. This should, at minimum, be applied to dynamic libraries, such as Windows DLLs, which are randomized at boot time so that the libraries are not loaded at predictable locations. This makes the process of finding a specific address to attack much more difficult, though not impossible.

﻿

This sort of protection is determined on a per-program basis, as the ability to use it must be determined at compile time. 

﻿

It is possible to enforce mandatory ASLR across all programs in an OS, and, in general, this can be a very effective control. However, it must be done with an understanding of how it may affect legacy software, as older programs that were not compiled to be ASLR compatible likely break when this blanket setting is applied.

![image](https://github.com/user-attachments/assets/d0d1bef1-3d79-4399-8769-d01cb55eb268)
﻿

For the reasons stated above, application of only systemwide settings that have been tested is helpful. However, when those have been thoroughly vetted, it is helpful to know how to check on ASLR mitigation compiled into existing programs, as demonstrated in the following lab.
...
Stack Exploitation Prevention 
The best way to prevent stack exploitation vulnerabilities from entering a defended system is by thoroughly testing and examining new patches and software before deploying them to production environments. This can take the form of source code review or patch testing in a sandboxed environment. In either case, it is important to understand the process and principles of Continuous Integration and Continuous Delivery (CI/CD).
...
The most effective, but certainly the most tedious, way to catch stack exploitation vulnerabilities before they are exploited in a production environment is to review the code of all new applications and their updates and patches. This is, of course, not always feasible. Some applications do not have publicly available code, and even for code that is available, a limit usually exists to the number of mission partner security engineers who have the time or expertise to read thousands of lines of code. 

﻿

With such constraints, the priority for code review should be applications compiled from a codebase that is not tested frequently in development or is not rigorously maintained, which may be the case for some projects maintained by the open-source community. For systems running these applications, more rigorous code review is necessary, and it may be helpful to delay updates that are not security related until sufficient testing is conducted. 

﻿

The following C functions are vulnerable when not implemented properly, such as reading into a buffer without checking the length of the input.

gets
scanf
sprintf
strcpy
This review process may be aided by compiler add-ins that highlight the use of these insecure functions and functions that are not bounds checked.
...
For an OS in which patches are frequently pushed (for example, Microsoft), a helpful security practice is to install patches in an isolated environment for a cool-off time prior to pushing the patch to the enterprise. In this environment, rigorous security and integration testing of enterprise applications should be conducted by mission partner engineers who are familiar with the code base employed and where potential overflow cases exist.

﻿

The type of testing normally conducted to install new patches or software may take the form of unit testing, integration testing, or functional testing.
...
Unit testing determines only whether individual components of an application function as designed. Unit tests may be run on individual input fields or message functions to ensure that, given a set of parameters, the application performs a singular task properly. Extensive unit testing of any component that takes user-supplied input is one of the strongest measures a security team can take to harden any applications hosted by a mission partner.
...
Integration testing ensures that various components and services of a multifunctional application work well together. For a web application, this may mean testing between a front-facing web service, a backend database, and possibly a security appliance, such as a web application firewall. Because the individual components were likely developed separately, the potential for unexpected input to be supplied from one application to another is high, but the permutations necessary to find a critically vulnerable case are high, making this testing important but tedious.
...
This type of testing merely determines whether the application or patch successfully performs the business function it is intended to perform. From a security standpoint as related to stack-based attacks, this type of testing is generally inapplicable though still important for an organization.
...
CI is the process of integrating the work of many different developers into a single project, usually through a shared code repository platform, such as GitHub. This integration occurs frequently and with built-in mechanisms to manage the conflicts between submissions from multiple sources. As many who employ this process automate the integration of the code, they also tend to automate its testing and correctness to assess code upon arrival. It is common to employ a fully automated testing server for this process to run the tests configured by the developers and return metrics on performance. In large and diverse teams, the use of CI has improved efficiency and delivery timelines of new versions and updates for many software projects. 
...
CD is the principle of maintaining a codebase that is always ready for deployment. By placing emphasis on stability and reliability, even between releases, teams develop projects that have fewer overlooked errors. This allows for rapid release of software versions and patches on demand, as the latest stable version is theoretically the one in the repository at any given time or requires few modifications to be both stable and reliable. 

﻿

This type of state is normally ensured through an approval process at a technical-lead level to ensure that integrated code bases meet the secure state expected of a project continuously delivering versions and patches to a user base. 

﻿

It is important for mission partners who develop software and adhere to the principles of CI/CD to ensure that any applications employed in the infrastructure are developed with these principles in mind or are thoroughly tested when the development philosophy is unknown. 
...
The downside to the CI/CD process from a security perspective is that overreliance on automation can develop. This may lead writers of code to overlook secure coding best practices, preferring to deliver volume and fix problems only when testing identifies them rather than to anticipate the potential security vulnerabilities before they are found. 

﻿

Additionally, in some open projects, certain vulnerabilities have been intentionally introduced by contributors, as the pipeline itself is vulnerable. This is why such projects and pipelines must be closely monitored to prevent irregularities from becoming threats delivered in live software. 
...
Privilege escalation is the process an attacker takes to exploit a bug, design flaw, or configuration errors in order to gain elevated privileges other than what was originally intended for the user. When this process occurs in a Windows environment, it is called Windows privilege escalation. If an attacker compromises a regular user account with lower privileges, they likely need an account with higher privileges to make system configurations necessary to maintain persistence and complete their mission. Windows is the most common OS in mission partner networks, and unauthorized escalated privileges in a Windows environment can lead to severe consequences. It is critical that a CPT analyst understands how to detect and prevent privilege escalation and which attack vectors should be closely monitored.
...
Because Windows is one of the most popular OSs, attackers may take multiple different vectors to exploit privilege escalation. Some common vectors include Unquoted Service Paths, Dynamic Link Library (DLL) Hijacking, and PATH variables. Attackers can also use such techniques as manipulating Startup Applications/Autoruns, User Account Control (UAC) Bypass, abusing the Windows SEDebugPrivilege, and exploiting Task Scheduler.       

﻿-------------------

 Unquoted Service Paths
Unquoted service paths arise when administrators fail to properly quote the full directory path to the executable that serves as the starting point for a system service. Attackers can take advantage of paths that lack quotations and contain spaces by placing a malicious executable in a higher-level directory within the path so that Windows chooses the attacker executable to launch. For example, if the path to a shortcut is C:\Program Files\app.exe, an attacker can add a malicious executable at C:\Program.exe, and Windows chooses this program instead. Attackers may use this vulnerability for privilege escalation if the executables are started by a higher-privilege process.

﻿

This is due to the way that Windows handles the CreateProcess Application Programming Interface (API) calls. If there are long filenames that contain a space, there must be quotes around strings to indicate where the filename ends and begins. Otherwise, the filename is ambiguous and the string can be interpreted in a number of different ways. 

﻿

For example, consider the executable path C:\Program Files\A Subfolder\B Subfolder\C Subfolder\App.exe.

﻿

To run App.exe, the system interprets this path in the following order:
...
5. C:\Program Files\A Subfolder\B Subfolder\C Subfolder\App.exe﻿

﻿

If C:\Program.exe is not available, then C:\Program Files\A.exe would be executed. Windows would go through the entire executable path, executing each .exe in the above example until App.exe was found and executed. This would allow an attacker to place a malicious executable where any A.exe, B.exe, or C.exe was located, and Windows would execute the malicious file as though it were App.exe.
...
A few steps should be taken to mitigate privilege escalation. One is to audit service paths and the registry for all services, especially those executing with administrator, or system, privileges. This includes cleaning up old Windows registry keys after software has been uninstalled and surrounding the paths with quotation marks when functions allow for them. Windows also has tools such as Windows Defender Application Control that identifies and blocks potentially malicious software that comes from an unknown source. One final step that should always be taken is to restrict file and directory permissions.  Ensure that Windows systems are using proper access control, and deny users the ability to write files to the directories C:\, C:\Windows\, C:\Program Files, and C:\Program Files (x86) so that malicious files cannot be placed there.
...
DLL search order hijacking is similar to unquoted service paths in that it relies on the vulnerability already being present from a misconfigured Windows system. Attackers can execute malicious payloads by hijacking the search order used to load DLLs. When an application loads a DLL without specifying a fully qualified path name, Windows attempts to locate the DLL by searching a defined set of directories in a particular order. Attackers can hijack DLLs in various ways. Attacks can plant trojan DLLs in a directory that is searched prior to the location of a legitimate library that is requested by a program, which causes Windows to load the malicious library. Search order hijacking uses the DLL’s record to spoof known DLLs. This record consists of a list of known DLLs on the current system and is stored at the registry key HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\KnownDLLs. 

﻿

Attackers can also perform DLL preloading by placing a malicious DLL with the same name as a DLL in a location earlier in the search order than the proper DLL for an executable with administrator privileges, such as a system service or autorun. Often this location is the current working directory of the program. If a search order program is configured to run at a higher privilege level, then an attacker-controlled DLL that is loaded is also executed at a higher privilege level. This could lead to privilege escalation from user-level to administrator-level or SYSTEM-level privileges. This type of attack relies on the attackers having write privileges to one or more of the locations in the DLL search path.

﻿

When a process or program tries to locate a DLL, Windows searches the directories in the following order:
...
3. The 16-bit system directory (default: C:\Windows\system).
...
Attackers can also execute malicious payloads by side-loading DLLs. DLL side-loading is similar to DLL search order hijacking because both techniques involve hijacking a specific DLL that a Windows program loads. Side-loading can be used to mask the actions performed under a legitimate elevated system or software process. DLL side-loading takes advantage of the Windows Side-by-Side (WinSxS) assembly feature, which helps manage conflicting or duplicate DLL versions by loading them from a common directory. WinSxS gives developers the flexibility to update binaries by replacing the old binaries in the same location, which greatly reduces the possibility of DLL version conflicts. Instead of planting a DLL within the search order, side-loading involves using the WinSxS assembly to load the malicious DLL from the SxS listing. Attackers then invoke the legitimate application that executes the payload. The SxS listing is located at the following paths:
...
Compared to other privilege escalation techniques, a DLL side-loading attack is often more difficult to detect by antivirus and other security software, and malicious DLLs can often be dropped without being detected. The key to discovering this vulnerability is to look for installation executables that have not been deleted from their original download location. Most of the reported vulnerabilities are in *.exe installation packages, and automated tools highlight anything in the current user’s Downloads folder that fits that description.  
...
Often the program that loaded the malicious DLL behaves normally because attackers can configure the malicious DLL to also load the legitimate DLL for a program. This makes this type of privilege escalation attack more difficult to spot. Mitigation for this technique is similar to privilege escalation. One step that should be taken is to use auditing tools capable of detecting potential DLL search order hijacking vulnerabilities. There are also tools that are included with Windows, such as sxstrace.exe, that can be used to check manifest files for problems in software. Another step that should be taken to identify and block potentially malicious software is to use application-control solutions, such as Windows Defender, capable of blocking DLLs loaded by dangerous software. There are also Windows group policies that can be configured to enable Safe DLL Search Mode, which forces the system to search for system DLLs in directories with greater restrictions and use them before local directory DLLs (for example, a user’s home directory). This group policy can be enabled from the Group Policy Editor:
...
The associated Windows registry key is HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\SafeDLLSearchMode.

﻿-----------------------------------

 ATH Variables
The PATH environment variable contains a list of directories used to search for executables. Certain programs, such as cmd.exe, PowerShell, or Python, rely on the PATH environment variable to determine which locations are searched for a program to execute when a path is not specified. Attackers can place a program in an earlier entry in the list of directories stored in the PATH environment variable with the same name as a Windows program. Windows then executes the program when it is searched sequentially through that PATH listing in search of the binary that was called from a script or the command line. If any directories are listed in the PATH environment variable before C:\Windows\system32\, a program can be placed in the preceding directory with a program that is named the same, and that program will be executed when that command is executed from a script or command line.

﻿

For example, if the path C:\lesson path precedes the path C:\Windows\system32 in the PATH environment variable, a program named Python that is placed in the C:\lesson path will be executed instead of the legitimate Windows system “Python” when the command Python is executed from the command line. This allows an attacker to execute their own malicious payloads on the system and use them to gain higher privileges. 
...
Similar to the above techniques, the best step to mitigate PATH variables is to surround the PATH variables with quotation marks, when necessary. This includes cleaning up old Windows registry keys after the software is uninstalled and using application control tools that report software using insecure path configurations, such as Windows Defender Application Control. It is also important to be aware of the search order that Windows is using and, whenever possible, use the fully qualified path. Another important step is to harden directory permissions in system folders to ensure that only the necessary permissions are granted, especially those that are of elevated privileges and to directories early in the PATH. System administrators should always ensure that users are denied the ability to write files in the top level directory C: and system directories such as C:\Windows\.
...
Autoruns is an advanced startup manager that is part of the Windows Sysinternals package. It allows administrators to manage programs that are loaded automatically at startup. Autoruns can display all the startup processes and programs that are configured for a Windows system, making it a powerful tool. 
...
If an attacker can gain access to a registry “run” key in the administrator’s autorun keys, this can be used to gain elevated privileges. By adding a program to a startup folder, attackers can add an entry to the “run keys” in the registry or startup folder, which causes the program that is referenced to be executed when the user logs in.

﻿

Placing a program within a startup folder causes that program to execute whenever a user logs in. These programs are executed as the user that logs in and have the associated permissions of that user as well. Each individual user on a system has a folder with programs that are executed when a user logs on, and the OS has a similar folder. These folders are checked when a user logs in, regardless of which user account it is. This technique can be used by attackers to achieve persistence on a target. 

﻿

The path to the startup folder for the current user is C:\Users\[Username]\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup.

 

The path for the startup folder for all users is C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp.
...
Run keys may exist under multiple hives. Registry run keys can reference different programs or list them as a dependency. The Cyber Defense Analyst – Basic (CDA-B) lesson Registry discussed the possibility of loading a DLL at logon using a “Depend” key with RunOnceEx: 
...
UAC Bypass
User Account Control (UAC) is an access control mechanism that ensures processes and programs run with the least privilege. If a process or program requires higher privileges, an administrator is required to authenticate the UAC prompt to approve the program or process asking for higher privileges.

﻿

If a user is logged in as an administrator and has the proper access, they will be able to answer “yes” to the UAC prompt and execute the program with elevated privileges. However, if someone is logged in as a standard user without administrator privileges, they will be prompted to authenticate as an administrator when the UAC prompt appears.
...
Attackers attempt to bypass Windows UAC in order to escalate privileges on a Windows system. UAC is a Windows security feature that allows other programs to perform tasks with higher administrator privileges. Depending on the privileges that the user has, UAC allows users to perform certain actions with administrator privileges or deny them if the privilege is not sufficient. If the UAC protection level of a computer is set to anything but the highest level, certain Windows programs can elevate privileges or execute some elevated Component Object Model (COM) objects without prompting the user through the UAC notification box. An example of this is to use rundll32.exe to load a specifically crafted DLL that loads an auto-elevated COM object and performs a file operation in a protected directory that would typically require elevated access. There are many different methods to bypass UAC to gain higher privileges, such as injecting malicious software into a trusted process like the Windows Connection Manager Profile Installer process cmstp.exe.

﻿

UAC Enforcement Levels
﻿

UAC has four levels of notification, each with different security levels, as described below.
...
If the notification level is set to Always notify, the system checks whether a secure desktop is enabled. Secure desktop helps to protect against UAC Bypass by presenting the credentials dialog box in a protected section of memory that is accessible only by trusted system processes. This is a high security level.
...
If the notification level is set to Default – Notify Me Only When Programs Try to Make Changes to My Computer, the policy setting User Account Control: Only elevate executable files that are signed and validated is checked.
...
If the policy setting is not enabled (the default setting), the PKI certification path validation is not enforced before a given executable is permitted to run. The group policy setting User Account Control: Switch to the secure desktop when prompting for elevation is checked.

This is a medium security level.
...
If the notification level is set to Notify me only when programs to make changes to my computer (do not dim my desktop), the CreateProcess is called. This is a low security level.
...
If the notification level is set to Never notify me when, the UAC prompt will never notify when a program is trying to install or make any change on the computer. This is not a recommended setting.
...
Mitigating UAC bypass involves ensuring the highest enforcement level and proper application of the Principle of Least Privilege to user accounts. The Principle of Least Privilege is the process of granting users only the permissions required to complete their daily tasks. This step is one of the most important to take to properly harden permissions on a system. This step also includes removing all users from the local administrator groups on systems and allowing only the domain administrators to perform administrator-level tasks. Windows systems should always be kept up to date with the latest patches and Windows updates in order to keep the Windows system safe.
...
Windows privileges are used to control security in Windows. A privilege is a right granted to an account in order to perform privileged operations. It is important to understand the difference between privileges and access rights. Privileges apply to system-related resources, such as access tokens, whereas access rights apply to securable objects, such as folders or files. 
...
SeDebugPrivilege allows a user to debug another process, which includes reading and writing to the memory of that process. SeDebugPrivilege allows the token owner to access any process or thread, regardless of the permissions. The debug privilege is a security policy setting that has legitimate uses, such as debugging of system components or remote components by developers and use of Microsoft Structured Query Language (SQL) Server by local server administrators. However, to the extent possible, SeDebugPrivilege should not be used. Attackers can use SeDebugPrivilege to duplicate the access token of a process with elevated privileges and use that access token in a child process. Once the access token is copied, it can be used to spawn a new process, like cmd.exe, with the privileges of the copied token.
...
The best way to mitigate the SeDebugPrivilege escalation technique is to disable the security setting when possible. Although the setting has some legitimate uses, it is a large security risk that should be avoided in most production systems. 
...
Attackers can abuse Windows Task Scheduler in order to execute programs at system startup or on a scheduled basis for persistence. These mechanisms can also be abused to run a process under the context of a higher-privileged account. One way this can be done on older Windows systems is with the at.exe utility, which is used to perform task scheduling at a specific time and date. If at.exe is used for an attack, the user to be logged on must be a member of the Local Administrators group and the Task Scheduler service must be running. In current versions of Windows, the at.exe utility has been deprecated.

﻿

Newer versions of Windows use schtasks for Windows Task Scheduler to perform task scheduling. This program can be exploited much in the same way that at was. Task Scheduler can be accessed in multiple ways. The schtasks utility can be run from the command line, or it can be opened through a User Interface (UI) within the Control Panel.
...
Mitigating these techniques involves being aware of Windows registry entries and having proper account management. To mitigate exploitation of the at program, examine the Windows registry entry at Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tree\At1, because Windows creates a registry key specifically associated with the creation of scheduled tasks at that location. Because at is deprecated, the following steps should be taken on all Windows systems to harden against Task Scheduler attacks. One step is to configure the settings for scheduled tasks to force the tasks to be executed using an authenticated account instead of allowing them to run as SYSTEM. This setting can be configured through the following Group Policy Object (GPO): 

﻿

Computer Configuration > Windows Settings > Security Settings > Local Policies > Security Options: Domain Controller: Allow server operators to schedule tasks, set to disabled﻿

﻿

The next step is to configure the Increase Scheduling Priority option to allow only the Administrators group the rights to schedule a priority process. This can be configured through the following GPO:
...
The last step is to limit the privileges of all user accounts and ensure that only authorized administrators can create scheduled tasks on remote Windows systems.


-------------------------

AlwaysInstallElevated
AlwaysInstallElevated is a Windows setting that allows non-privileged users the ability to run Microsoft Windows Installer Package Files (MSI) as SYSTEM. This setting is helpful to local administrators for installation deployments but poses a large security risk to the local Windows system. If this setting is turned on, attackers can create their own malicious MSI and execute that MSI on a system in order to escalate their privileges. Having this setting enabled is equivalent to granting administrator rights to non-privileged users. 

﻿

For an attacker to exploit this setting, the following registry entries must be set to a value of 1: 
...
These settings are not turned on by default, but some administrators may have them enabled in order to allow users to manually install packages that usually require higher privileges. The best way to mitigate this technique is to always ensure that the AlwaysInstallElevated registry setting is set to 0. Administrators can deploy the software via GPO or other third-party solutions instead of using this Windows feature.
...
Detecting privilege escalation in a Windows environment involves monitoring and thoroughly auditing Windows logs and events. Monitoring file creation and process creation events, especially those with the same name as executable files run by administrator-level services, is an important process to detect and stop privilege escalation. If such a file is created by a lower-privileged user or a process is spawned outside an expected path, an investigation is warranted. This is why it is important to understand the proper paths that files take to execute. 

﻿

Monitoring file systems for moving, changing, or renaming DLLs is another important step to take to detect privilege escalation. Changes in the set of DLLs that are loaded by a program or process that do not correlate with known software or patches are suspicious and should be cause for investigation. Monitor DLLs that are loaded into a process, and flag any events or DLLs that have the same filename but are loaded from an unexpected directory.

﻿

Along with file systems and system events, the registry should be monitored for changes to run keys that do not correlate with known software or Windows patches. Along with registry keys, the start folder should be monitored for additions or changes in order to detect system changes that could be attempted at persistence. Using such tools as Sysinternals Autoruns can assist in detecting changes and suspicious programs execution, as startup programs can appear as outlier processes when compared to historical data.   

﻿

Data or events should not be viewed as isolated but, rather, as part of a chain of behavior that could be part of an attack. Network connections made for Command and Control (C2), learning details about the environment through discovery, and lateral movement are all avenues that attackers take during an attack, and these should be part of the thought process used to detect privilege escalation.
...
Data exfiltration is the process of removing sensitive data from a compromised target environment to a threat adversary-controlled space. Adversaries commonly follow successful exfiltration with activities such as ransom, exposure, or analysis.
...
Adversaries use many different network communication protocols for data exfiltration. These protocols and techniques vary, depending on how the adversary chooses to avoid detection by network defenders. The techniques adversaries execute over common protocols often employ some form of obfuscation or encryption to prevent analysis. Adversaries may also try to prevent data loss during exfiltration with stateful connection protocols, such as services communication over the Transmission Control Protocol (TCP). Another option to prevent data loss is User Datagram Protocol (UDP) communication with custom message control. 
...
A common method for exfiltrating data from a system is to gain a foothold in a compromised environment, first. The easiest way to do this is through an implant or vulnerable process communicated over an encrypted Command and Control (C2) channel.

﻿

Exfiltration over the C2 channel can involve simply printing the contents of a file to standard output, which sends the contents over the C2 channel to print to the attacker’s screen. The attacker can then manually copy and paste the contents into a local file. However, not all data encodes cleanly into American Standard Code for Information Interchange (ASCII) or Unicode characters. In these cases, attackers require a much more robust file transfer mechanism. Since this need is well known, file transfer functions are built into modern attack frameworks, such as Empire and Metasploit. These frameworks exfiltrate a file’s binary data over the C2 channel with simple commands. 

﻿

However, even when a C2 channel exists, an attacker may still choose to use a different protocol for data transfer to reduce the footprint of that channel’s communications on a network. Using a different protocol over the C2 channel also allows attackers to separate interactive and automated exploitation and exfiltration activities. This separation prevents defenders from identifying threat activity by signature of the transportation channel alone.
...
Outbound connections to random or non-standard ports, such as 5555, are certain to draw a defender’s attention. However, adversarial communications over established internet services and protocols, such as web and email, appear normal in any network. Attackers using established protocols for data exfiltration are likely to remain undetected for a longer time after compromise, since the traffic is disguised as expected and permitted. This technique requires an especially close inspection from defenders to detect suspicious actions. When these established protocols are used for data exfiltration, they are referred to as “alternative protocols,” which include the following:
...
The original web communication standard is the Hypertext Transfer Protocol (HTTP). It is an attractive option because it is one of the most ubiquitous and permitted protocols in the world. The downside for attackers using HTTP is that its contents are plaintext by default. This means that anything an attacker obscures through encryption is immediately suspicious to defenders and requires further inspection. However, even encrypted data is still generally lost in the noise of simple volume. Unencrypted HTTP communications are much less common now, since most modern browsers attempt secure protocols before moving to the less-secure, unencrypted HTTP protocol.
...
HTTP Secure (HTTPS) communications are the most likely alternative protocol that attackers employ in modern data exfiltration efforts. HTTPS is essentially as common as HTTP communications in normal traffic and is expected to be encrypted by default. Careful inspection is still required to identify suspicious attacker traffic. For example, an organization may employ Transport Layer Security (TLS) inspection by intercepting the HTTPS handshake so that all HTTPS traffic may be read for security purposes. In this case, defenders flag traffic that remains encrypted. However, an attacker may circumvent even these mitigations by exploiting policy. Some organizations exempt TLS inspection of healthcare-related communications to avoid violating patient rights or compromising healthcare information for legal reasons. If an attacker can spoof or domain-front from an exempted healthcare domain, then even TLS inspection does not reveal HTTPS data exfiltration as suspicious. Identifying suspicious or abnormal traffic in this type of communication depends on destination and domain analysis.
...
Organizations have used File Transfer Protocols (FTP) for file transfer operations since the dawn of the Internet. File transfer communications, by themselves, are not suspicious. However, transferring to servers outside the organization, especially if FTP activity is otherwise abnormal, may raise red flags. Attackers may also use related FTP protocols: Trivial FTP (TFTP) and Secure FTP (SFTP). Modern web browsers are deprecating support for FTP, which is the primary end-user case for FTP. This type of traffic is likely to decline as other protocols take its place.
...
The Simple Mail Transfer Protocol (SMTP) is used to send emails between parties. Attackers have implemented SMTP to send data from a compromised system to an account under their control, in gray space. In the past, attackers often used a vulnerable open relay to spoof an innocuous email address. More recently, threat adversaries have been using common email providers such as Gmail.
...
Domain Name Service (DNS) data exfiltration is unique. It allows data transfer between two devices without a direct connection since the request occurs over UDP through intermediate DNS servers. During the exfiltration phase, a compromised client makes a DNS resolution request to an external attacker-controlled DNS server. Instead of responding with an A record, the attacker’s name server responds with either a CNAME, MX, or TXT record. This allows a large amount of unstructured data to be sent between attacker and victim.
...
The Server Message Block (SMB) protocol is commonly used in Windows environments to provide network access to common resources. This may be used for file transfer by hosting a SMB network share on the attacker or victim machine, then using the protocol to transfer files between them. A future lesson provides more information on exfiltration using this technique. 
...
Attackers use Internet Control Message Protocol (ICMP) to tunnel malicious traffic and data exfiltration. They achieve this by injecting data into an ICMP echo or echo reply. These packets may include a data field that attackers use for data exfiltration. If a defender observes extensive ICMP usage over a network or abnormally large ICMP packet size, this is cause for concern and further inspection.
...
This category includes obscure or deprecated protocols such as finger or telnet. Attackers do not often use these protocols for data exfiltration due to their uncommon use and plaintext communications. However, if a defender is not monitoring these miscellaneous protocols, attackers may consider them an option.
...
Attackers may use one or more of the following methods to smuggle out data from an environment. This may be in addition to data exfiltration using the network protocols previously mentioned, or as an implementation of those techniques.
...
Simply putting the data in the packet body of protocol communication is the simplest and most direct method of transferring data. An example of this is using the body of an HTTP post request or an FTP upload. This is the expected location for data in those communications. An intelligent inspection of that data that uncovers nonconformity to the protocol standards or unexpected information may raise red flags. However, this method has traditionally involved the least additional overhead in terms of creativity or programming new techniques. The tools and standards for transferring data with this method are built into existing tools that most operating systems often include by default as part of their installations.
...
Attackers have different options to avoid exporting data in plaintext. They can encode the files or data. Another option is to include the data in the header fields of the protocol or the metadata of the service communication standard. 

﻿

Smuggling data in the header fields requires a few steps. For example, an attacker must first encode the data with the Base64 algorithm. The attacker then breaks up the data and send a series of GET requests to an attacker-controlled web server. In each request, the attacker includes the data in a header field that normally expects a random value, such as the Cross-Site Request Forgery (CSRF) token, which is normally used to prevent CSRF.
...
Attackers may opt to hide exfiltrated data completely by encoding it with steganography into otherwise innocuous and expected forms of data, such as images uploaded to social media. Attackers have used steganography to both exfiltrate data and also operate as a covert channel for C2 infrastructure. 

![image](https://github.com/user-attachments/assets/ad7912b1-4348-465e-807b-02aa5bfa259c)
...
Data Exfiltration Mitigation Strategies
Mitigation Strategies
﻿

According to the MITRE ATT&CK Framework, a combination of Data Loss Prevention (DLP), network traffic filtering, and network intrusion prevention creates the strongest mitigation against data exfiltration techniques.
...
DLP is a set of tools and processes used to ensure that sensitive data is not lost, misused, or accessed by unauthorized users. Defenders can use DLP to detect and block sensitive data being uploaded through web browsers.
...
Defenders filtering network traffic should enforce proxies and use dedicated servers for services such as DNS. They should also only allow dedicated servers to communicate over respective ports/protocols, instead of all systems within a network. Additionally, defenders can use TLS inspection and automated rules to find data that is encoded after the handshake or with an algorithm that is not a TLS standard. HTTPS traffic that is not encrypted with a standard TLS algorithm is an immediate red flag for defenders.
...
Defenders can use network intrusion detection and prevention systems to mitigate activity at the network level. These systems use network signatures to identify traffic for specific adversary command and control infrastructure and malware. Defenders can write Suricata rules to alert on the use of particular user agents. For example, alerts can be set for the PowerShell user agent that attackers use to perform an HTTP POST request. 
...
SMB Share Enumeration Overview
Many networks contain shared drives to enable easy, convenient collaboration between employees, allowing for smooth operation among a distributed workforce. Attackers search for shares on systems to locate potential targets for data exfiltration, identify potential privilege escalation targets, and identify points of ingress for lateral movement. 

﻿

Asset identification and discovery techniques are varied and can sometimes take place over weeks or even months. Once a target is identified, adversaries use one or more techniques in a sophisticated attack chain sequence to gain access and then move laterally throughout a network.

﻿

Aside from targeted scanning, like vulnerability scans from a security team, network discovery–related traffic is rare. Most administrators are aware of what is connected to their network and, outside standard scanning, do not need to perform regular discovery checks. Windows command-line arguments from tools like the NET command may be indicators that an attacker is attempting to gather system and network information. Such remote access tools as Remote Desktop Protocol (RDP) interact directly with the Windows SMB and Remote Procedure Call (RPC) protocols, which can be used to gather system information and manage systems remotely. Other administrative tools, such as PowerShell, may also be used to quickly identify shares and network systems.

﻿

SMB is tightly integrated into the Windows Operating System (OS) and provides many services users rely on, including file and directory sharing, sharing of peripherals (for example, printers), and other resource sharing. Over the history of the SMB protocol, many critical vulnerabilities (such as the EternalBlue series of exploits, discussed in the Cyber Defense Analyst – Basic [CDA-B] course) have been identified and patches have been developed. However, some of those many patches may not be installed on every system in a complex network. Enumerating SMB-enabled systems can identify open shares and other flaws and vulnerabilities that an attacker can take advantage of.

﻿

Some common SMB enumeration tools available to the public include the following:

Nmap Scripting Engine (NSE) SMB scripts
...
All these tools use the SMB, or RPC, protocol to interact with a system to provide insight into the configuration and state of a Windows host. Defenders need to understand how attackers use SMB enumeration as a leverage point in laterally moving between Windows hosts in a compromised network in order to identify and defend against this activity.

﻿

SMB Share Enumeration Detection
﻿

Once an attacker has gained a foothold into a target network, they use a number of methods to identify potential targets. Most commonly, they use common OS commands found on the first system they infiltrate to identify other targets.

﻿

NET Command SMB Share Enumeration Example
﻿

The NET utility is built into the Windows OS. It is used in command-line operations for control of users, groups, services, and network connections. NET is capable of gathering system and network information for discovery, moving laterally through SMB/Windows admin shares using net use commands, and interacting with services.

net view /domain[:DomainName] /ALL
﻿


This command outputs all the SMB shares in a given Windows domain. The domain name can be found in a number of ways by an attacker, including the techniques discussed in the Cyber Defense Analyst – Host (CDA-H) lesson Active Directory – Enumeration.

![image](https://github.com/user-attachments/assets/22c7a4c6-7751-4a50-b31a-85c00ba91a09)
...
![image](https://github.com/user-attachments/assets/3766f8ab-68ec-4ffa-ac25-535d49629097)
...
Hunting for SMB Enumeration
NET.exe
﻿

To detect NET.exe activity, Windows security event ID 4688 — a new process has been created — or Sysmon ID 1 — process creation — is used. These event codes show the exact command that was run by the user or adversary. Determination of malicious or normal activity is based largely on the type of account running a net group command. A non-admin account running a net group command to list all the users in the Domain Admins groups is quite suspicious, given that most users — with the exception of a relatively small subset of power users — do not know how to do this. An admin running the command is much less suspicious, and more context is needed to determine if the activity is malicious.

﻿

Another way to determine if NET.exe activity is suspicious is by examining its usage over time and creating a baseline. One red flag would be the observation that a certain user has not used the NET.exe command in the past 6 months but now they are enumerating domain admins and network shares. Time graphs may also be used to examine usage and determine if it is automated/scripted or human activity. Human activity is much more scattered than machine activity; automated activity usually has some semblance of structure. For example, if NET.exe is being run every hour, on the hour, automated machine activity — and perhaps an admin script — is likely occurring. This can be verified on a system by checking with the local admin or investigating the script or scheduled task that is triggering the activity.
...
Nmap scans can be difficult to detect, depending on the sophistication of the threat actor. Examining Intrusion Detection System (IDS) or other packet sniffer logs can reveal Nmap scans if the adversary is creating a large amount of traffic and uses the default Transmission Control Protocol (TCP) Windows setting. One potential indicator of a scan is if one host in an environment has a sudden spike in outgoing traffic.

﻿

The Nmap Scripting Engine (NSE) has 35 SMB enumeration and exploitation scripts. Nmap NSE scripts are typically loaded in the /usr/share/nmap/scripts directory on Linux systems and the scripts subdirectory of the Nmap installation on Windows systems. A lower-threat adversary may use these scripts, or other easily detectable methods, to probe and enumerate Windows hosts. Higher-skilled threats often use custom tools that may not already have published alerting rules. The following list contains the most common of these scripts that can provide significant details of a Windows host, all of which should be identified by IDSs. NSE scripts are executed after the initial portion of an Nmap scan has completed identifying the hosts that are network accessible. If performing a targeted scan, for example, for SMB-enabled hosts, the best practice is to limit the Nmap scan to only the port or ports associated with that protocol. 

﻿

An example command to execute the smb-os-discovery.nse script on hosts that have the SMB protocol is as follows:

nmap -p 139,445 –-script smb-os-discovery.nse [Target IP address/range]
﻿

The following is a list of common SMB scanning scripts available in Nmap for different types of SMB enumeration:

smb-os-discovery.nse: Identifies the OS version on potential targets. Adversaries can use this to find end-of-life systems or higher-value targets like servers.

smb-enum-shares.nse: Lists shares, which an adversary can use to look for valuable info to exfiltrate.

smb-enum-users.nse: Enumerates user accounts with higher privileges, which are invaluable for lateral movement.

smb-enum-domains.nse: Enumerates domains, which allows an adversary to find out how a network is segmented and organized.

smb-enum-groups.nse: Discovers user-created groups, which can lead to more targeted user exploitation.

smb-enum-processes.nse: Enumerates running processes, which can show possible vectors for an adversary’s attack chain.

smb-enum-services.nse: Enumerates services, which can lead to further exploitation.

smb-enum-sessions.nse: Enumerates active user sessions, which can potentially be hijacked for lateral movement.

NBTscan
﻿

NBTscan is a tool that sends a NetBIOS query to IP addresses in a given range and displays the received system information in an easily parsable format. With each query, NBTscan can obtain IP addresses, NetBIOS names, current active user account names, and hardware Medium Access Control (MAC) addresses.
...
SMBMap is a common pentesting tool that returns SMB host information in a format specifically designed for identifying attack points. Although much data can be gathered from native OS tools like NET, SMBMap goes a step further to include more detailed permission and potential vulnerability-related data. An example of the output of SMBMap executing with credentials is shown in Figure 10.2-4.

![image](https://github.com/user-attachments/assets/65422e39-f709-4d8a-8885-34bde7e032d1)

RPCclient


RPCclient is a Unix tool built into Samba that allows administrators to manage Windows-based hosts from their Linux/Unix machines. It bundles many of the system calls and commands that Windows administrators use to manage and query the assets they manage without the need to be directly tied to the target domain.


The following exercise includes scans to examine and analyze using some of the SMB enumeration tools discussed in this section. 
...
Lateral movement occurs when adversaries use various techniques to move within, and exploit additional systems on, a network. Adversaries attempt to discover which targets are viable for exploitation and then attempt to attack those hosts to move throughout the network. Often, lateral movement requires multiple attacks to be chained together across various systems to reach the intended target. As an example, an adversary might compromise a user's system through phishing or remote exploitation. The adversary might then move through the network until they can find a weak point to a more valuable target, like a file server or a DC. This allows them to largely use internally trusted resources to maintain a stealthy foothold within the network before they begin exfiltrating data or performing other Malicious Cyberspace Activity (MCA).

﻿

Once inside the network, an adversary searches for systems with remote services like Secure Shell (SSH), WinRM, Windows Management Interface Command (WMIC), and other SMB-based utilities, as such services allow direct connection to those systems directly. Attackers try to achieve several primary goals with lateral movement:

Persistence
Command and Control (C2)
Exfiltration
Targeted MCA
Systems that have long uptimes and direct access to the internet are prime candidates for persistence, C2, and exfiltration. Other candidates for persistence include little-used networking systems and user systems that can intermittently beacon to an external C2 server, blending in with legitimate user activity. A stable system that has little or no logging is often desired for internal (that is, within the compromised network) C2 and exfiltration. These systems are usually servers. Attackers often seek out UNIX systems because fewer users typically use them directly, and they often have a larger normal network traffic throughput baseline. The larger network traffic throughput baseline allows easier exfiltration of larger amounts of data while blending in with normal activity. The adversary might have obtained legitimate user credentials through other methods, or they might exploit vulnerabilities in the remote session protocols to take control of the system.

﻿

Adversaries might also embed malicious code, such as a financial spreadsheet, in shared content so that when the infected document is shared within an organization, the adversary gains a foothold on each new user machine that opens it. This allows them to hunt for different privileged users, as many local exploits take advantage of the current user account. For example, if a Human Resources (HR) employee shares a document with an Information Technology (IT) employee, the adversary gains far more access than they had with only the compromised HR employee’s system. Once the adversary has access to a higher level of privileges, they can even use internal software deployment and update applications to distribute malware through an entire network, hidden within a trusted update process.
...
Adversaries use integrated system tools as part of their malware packages when performing lateral movement as part of their attack chain. Such tools include the following:

PsExec: A command-line utility included in the Sysinternals Windows suite, used for executing commands on remote systems. It may also be used to launch interactive command prompts.
SCP (Secure Copy): The UNIX Command-Line Interface (CLI) tool for transferring files between hosts. It may also be used to send malware laterally across the network. 
SSH/WinRM/SMB/WMIC/RPC/RDP: The remote session protocols for Unix and Windows. If an adversary can hijack active sessions or connect on their own, they can move between hosts with ease.
Lateral movement is part of nearly every adversarial goal. It allows adversaries to increase the amount of damage they can do to more valuable targets and become more difficult to detect on the network.

![image](https://github.com/user-attachments/assets/57072fa0-d9eb-45d0-a056-7dbe6e80ad96)


----------------

Remote Services Review
Remote services are various protocols that allow users to perform actions on remote hosts. Users can log into other hosts with RDP or SSH sessions, share files using SMB or SCP, or perform full remote administration with tools like PowerShell using WinRM and PsExec.

﻿

Remote Service Use in Lateral Movement
﻿

Administrators regularly use remote service tools as part of their daily duties. This means that these common tools are likely to appear in system logs and do not appear out of place even when an adversary uses them. Adversaries use these tools to open other systems to further exploitation by elevating their privileges, opening ports, starting services, installing malware, hijacking sessions, and more. One indicator of potential malicious traffic is the use of remote service tools between user machines or non-administrative devices. An attacker may take advantage of something like a network printer, which typically has an embedded version of Windows or Linux, to laterally move to, or perform administrative commands on, other assets in the network.
...
MITRE Adversarial Tactics, Techniques, and Common Knowledge (ATT&CK)® describes exfiltration with the following characteristics:
...
After an adversary has established a presence on a network, the next step in the campaign is discovering useful data. The data can include artifacts such as accounts, host names, application logs, emails, and instant messages. The next step is to create avenues to capture the data and send it to an external host or location where they can analyze and use it to further the campaign. The adversary may complete this step using either cloud or web services to exfiltrate data to a specific location.

---------------------

Data Exfiltration Techniques
Adversaries may use different techniques to leverage web and cloud services for data exfiltration. Some common techniques include exfiltration over web services, exfiltration to cloud storage, and exfiltration to code repositories.
...
Web services are protocols, standards, and components accessed remotely through the internet, from machine to machine. Web services include software, applications, and cloud infrastructures that use common web protocols such as Hypertext Transfer Protocol (HTTP) and Hypertext Transfer Protocol Secure (HTTPS) to facilitate communications and connections. Some common web service entities include Amazon Web Services (AWS) and Google. 

﻿

A common technique for data exfiltration is the use of web services as a transportation method. The adversary may use a legitimate web service to transit stolen data from compromised hosts using common services such as HTTP or HTTPS. From an adversarial perspective, a primary benefit of using web services is that the service is often already allowed through the perimeter and blends in with expected traffic.
...
Cloud services include storage, software, applications, and infrastructure accessed over the internet. A third party that is external to the connecting entity typically hosts these services. Some common cloud service entities include Dropbox, Google Cloud, and Microsoft Azure. Communications to cloud services can be a legitimate and necessary requirement for some organizations. Cloud services may provide remote accessibility, additional resources, and backup for critical data. 

﻿

Adversaries may use cloud services to transport the data they exfiltrate, similar to web services. They may also use legitimate cloud services as destinations for data they stole from the exploited host. In this technique, the trusted cloud storage service replaces the typical command and control (C2) channel. Organizations naturally grant an allowlist distinction for the cloud services their teams use, so any connections and communications to the service are trusted and allowed. 
...
Another common technique for data exfiltration is by using code repositories. Code repositories are locations where developers and organizations can assemble and maintain code and software. The adversary may attempt to use a legitimate code repository (such as GitHub) to send stolen data from the exploited host. Similarly to the web or cloud services, connections to the repository may be allowlisted. 

![image](https://github.com/user-attachments/assets/2c135d97-0eab-4fa0-87bc-8908d6f0deb2)

![image](https://github.com/user-attachments/assets/b26ed569-ee7a-40d0-a484-d2cc9f94ec20)

-------------------

Allowlisting Cloud Services
Allowlisting Overview
﻿

Allowlisting, is the practice of allowing user or computer access to essential components such as software, applications, Uniform Resource Locators (URLs), Internet Protocol (IP) addresses, or cloud infrastructures. Allowlisting is the explicit denial of the components not on the approved list. The strategy denies everything except the specific definitions and parameters an organization permits. Denylisting is the opposite. Denylisting permits every component, except the definitions and parameters an organization explicitly blocks. 

﻿

Risks of Allowlisting
﻿

While it may be convenient and efficient to use an allowlist to facilitate access, this also hides adversarial activity on allowlisted components. Allowlisting informs a network to expect connections and communications to specific components. As a result, the network does not detect or identify adversarial activity on allowlisted components as unusual or threatening. 

﻿

Another risk with allowlisting is in using definitions and parameters that are too wide in scope. For example, by allowlisting all of AWS. This makes all AWS components available to threat actors to use to their advantage. Allowlisting broadly, rather than allowlisting selective, compartmentalized areas increases an organization’s exposure to possible exfiltration. 

﻿

Allowlisting cloud services increases the risk of exfiltration occurring undetected. Without careful consideration of these risks, security teams may not realize the need to conduct closer and more frequent inspections of connections and communications to allowlisted locations. The table below summarizes some of the advantages and risks of allowlisting that are worth considering.

![image](https://github.com/user-attachments/assets/9c55840d-4a82-4395-8992-e56ee819de34)

![image](https://github.com/user-attachments/assets/0372be13-7cf0-4591-9641-56e90457321f)
...
When a team coordinates a hunt for an attacker exfiltrating data through DNS exfiltration, they will need to review and analyze the network traffic logs they’ve collected. Packetbeat by Elastic is a tool that compiles and sends network traffic to a Security Information Event Manager (SIEM). Packetbeat functions similar to Winlogbeat, but specializes in the activity, requests, and connections made on the network. Packetbeat sends the data to Logstash or Elasticsearch for analysts to search more easily. 

﻿

Below are some of the common Packetbeat fields associated with DNS traffic. The following labs within this lesson leverage these fields to uncover DNS exfiltration data. 

﻿
![image](https://github.com/user-attachments/assets/ff1cf483-6918-4165-8ce6-acd2b3b6b313)

Hunting for Domain Name Service (DNS) Exfiltration


One method of DNS exfiltration is by using subdomains to carry stolen data. For example, the authoritative DNS server google.com processes a DNS request made for hiddendatahere.google.com. If an attacker controls this authoritative server (which exists outside the victim network), they can use it to capture the hidden data being sent as part of the subdomain in the DNS request. This allows an attacker to exfiltrate data simply by making DNS requests. The attacker does not require a direct connection to the attacker server.


Attackers often use an encryption or encoding scheme to hide the data and transfer it through DNS without errors. For example, an attacker looking to exfiltrate the string helloworld over DNS might make a DNS request for 68656c6c6f776f726c64.google.com. This request contains the string for helloworld in hex-encoded American Standard Code for Information Interchange (ASCII) as the subdomain in the request. The attacker can then convert this hex string back into ASCII to retrieve the initial data that was exfiltrated. Attackers also implement their own custom encryption or encoding methods. This makes it nearly impossible for a local defender to extract the data being sent as part of the DNS request.


Adversarial activity often causes a spike in DNS traffic on the network. That’s one of the first signs that suspicious activity may be occurring. The uptick in traffic may also occur when there is usually little activity occurring on the network, such as late at night or early morning. This helps adversaries avoid immediate detection by security teams. 
...
PrivateBin Exfiltration Summary
The network traffic and data exfiltration over the PrivateBin cloud service is not obvious. There are a couple of red flags that should alert a defender to analyze the traffic in detail for potential MCA. The first is a non-standard user-agent making network connections: Go-http-client/1.1. 

﻿

The second red flag is the multiple, repeated internal HTTP POST connections to the remote server. While both GET and POST HTTP connections can send data to a remote server, POST connections can upload greater quantities of data. POST connections are not by themselves abnormal, since many web forms use POST connections to send the results of the form to the server for processing. 

﻿

Figure 10.3-21 below shows a query to identify potential MCA data associated with PrivateBin:

![image](https://github.com/user-attachments/assets/c1c4f75d-1f81-4caf-be58-bbafa2084dd7)


Go is a compiled programming language that is increasingly used for network communications. Due to its ease of use, attackers also take advantage of Go's syntax to create command and control (C2) and exploitation frameworks. The results returned from the query in Figure 10.3-20 identify GET requests using a Go user-agent which may indicate a C2 framework repeatedly requesting a page from PrivateBin that contains instructions for the agent to complete on a compromised host. The data sent to PrivateBin over the POST connections is encoded and encrypted. The content-type shows the data was urlencoded, which makes it harder to analyze. Using a tool like CyberChef, the data can be partially decoded. 


The following is the original encoded data:
http://privatebin.com/?burnafterreading=0&data=%7B%22iv%22%3A%22p0wxflhkxEo9W7DRhbBzDw%3D%3D%22%2C%22v%22%3A1%2C%22iter%22%3A1000%2C%22ks%22%3A256%2C%22ts%22%3A128%2C%22mode%22%3A%22gcm%22%2C%22adata%22%3A%22%22%2C%22cipher%22%3A%22aes%22%2C%22salt%22%3A%226rdDqpP7MSc%3D%22%2C%22ct%22%3A%22V%2Fr6MDEWbnNLNrj0M4mXGaFG0N%2BPa%2BxT4bdeq%2BlNSqyuxA3twZsKBoUqRSx9MdF09AI8i19baZA7%2BpYxxUXNfAqWb1F6gLlhPTRxwK8nb7yQmm1BX2YQz3x3TJVOvxM%2FIShGwuvAbxJ1Dl%2B6lUQUmwBoEAd5%2Fchx0MFjaen9rFCzAo2Uh4m%2FCwlsJudBLvKUYjXZOwBt7oWK7HpS0dYPocM%2FZGVAJlZxrfZdctWHZzniHdxxQguR%2F2QJ%2FLV49gMazWEyaPCFVW7CVbDFvTNbsoibyBVc05aenI7oGDEhO2SKvxgr%2FR%2BQeAzhvTO3takfKvLPYni5d3XXS%2BP32NJulRhp8PsY%2BNrQcgXbSRknNIPl7Ec48WWeuprTCrHdP9tec%2FCyQ1%2FpIuxmT8eQAH4FVnB2EiAwxqUUuS96%2FKPM7LOZjonBoTlOpg27v%2BC4KkmE7uXxOGHd%2BGrVRKMsCwhx6yvC7xlAHpY5RfkBo99uCs2MCMZyZBnXAGX5SsvgiDBZah8HTSk36v1qN1kFA0NJwXd%2Bse1LtWd9Avr0TqlUB4cJiWAIHnAD8rbyfmHRxHdz06pSfMOzloe47JLgi5TQasVkev26VnQ3UWq98sB1uixJlmxeE4bxGN%2BwdhEkaBCeZHMWspHHE0k%3D%22%7D&expire=1week&formatter=plaintext&opendiscussion=0



Below is the decoded data:
http://privatebin.com/?burnafterreading=0&data={"iv":"p0wxflhkxEo9W7DRhbBzDw==","v":1,"iter":1000,"ks":256,"ts":128,"mode":"gcm","adata":"","cipher":"aes","salt":"6rdDqpP7MSc=","ct":""}&expire=1week&formatter=plaintext&opendiscussion=0



The decoded data shows encrypt ion-related variables and reveals that AES was used as the cipher suite to encrypt the data sent to the PrivateBin server. This data does not provide the defender enough information to decrypt the data that was exfiltrated. However, these indicators are sufficient for a more detailed host investigation to hunt for the specific processes that initiated this traffic. Without the additional detail of the processes involved, this activity is still considered suspicious, but has not necessarily been determined to be malicious. This scenario did not show any additional HTTP traffic which normally exists in a network to highlight just the relevant data. Identifying this data is much harder in an operational environment and requires a clear understanding of the normal, baseline activity for each allowed cloud service.
...
Cloud Services
Figure 10.3-27, below, displays how adding the field dns.question.name to the Discover table displays that Azure was the cloud service exploited on the ch-tech-3 host.

![image](https://github.com/user-attachments/assets/e9f71a37-e133-4217-ad63-fcd014965e14)


The connections made to azure.com include a large am ount of alphanume ric text prior to the subdomain. The text is indicative of using the subdomain to send data in hexadecimal via DNS traffic. A DNS request  for 6865 6c6c6f776f726c64.34*.azure.com contains a hex-encoded ASCII string in the subdomain in the request. 

---------------------------

Investigation Summary
A portion of the investigation into the attack that was performed against the VCCH network was just completed. The investigation uncovered that the adversary had access to the ch-tech-3 machine, and was exfiltrating data via DNS requests to azure.com, an allowlisted cloud service.

﻿

In a real-world situation where MCA is identified, hunt and investigation actions likely result in a clear operation to remove the attacker from the network and restore it to full operating capability. In this example, properly clearing MCA  from the  ch-tech-3 machine and the larger VCCH network requires additional investigation to occur. This investigation includes searching for how an attacker gained initial access into the VCCH network, the exact malware that was leveraged, what files were stolen from the network, and other attacker actions that may have occurred within the network.
...
Physical Access Exfiltration Methods
Overview
﻿

The unconventional methods in this lesson are often the last resort for a threat adversary who seeks to exfiltrate data from a target environment. Each of these methods requires physical access either to deliver a payload that creates a custom exfiltration vector or to place a device that removes the target data. The methods in this lesson include:
...
Out-of-Band
There are many ways to describe out-of-band exfiltration. For the purposes of this lesson, this refers to radio frequency bands used for alternative wireless communication. These bands allow data exfiltration from physically isolated or internet air-gapped networks.

﻿

Other common descriptions of out-of-band do not apply to this lesson. For example, any protocol that is not a standard stateful, synchronized transport method. Examples of these transport methods include Hypertext Transfer Protocol (HTTP), Secure Shell (SSH), and Transmission Control Protocol (TCP) through Netcat.
...
Any type of device that is not intended to operate in a mission partner environment is a rogue device. These are often added by malicious threat actors or their human proxies when they have physical access to a target space. How the device functions depends on the type of device. A rogue device can serve as a physical tap, a wireless access point, or even as a workstation that blends in with normal network traffic.
...
Removable media such as the convenient and accessible Universal Serial Bus (USB) flash drive can also be a huge security risk. People can use them to compromise a system that is poorly configured or to physically remove data when no other methods are available. Proper logging can detect this form of exfiltration by recording when files are moved onto flash drives.

![image](https://github.com/user-attachments/assets/9e15eda0-72d0-4439-87aa-92f5deb919b9)
...
Advanced Persistent Threats (APT) may be able to exfiltrate data wirelessly outside of recognized protocols and radio frequencies. This is possible if the APTs are extremely proficient, use highly engineered custom firmware, and have access to sensitive intelligence about the technical internals of their target network.

﻿

The MITRE ATT&CK Framework identifies this technique as “Exfiltration Over Other Network Medium” and describes it in the following way:
...
Below, Figure 10.4-1 illustrates the MITRE definition for this type of exfiltration within an air-gapped network. Compromising air-gapped networks is difficult, but not impossible. When an air-gapped network is not connected to the general internet, attackers must employ unconventional means of exfiltration, such as the following:

A physical device

An alternative radio frequency that may be active (such as Bluetooth®)

A custom wireless signal created by highly skilled attackers who have knowledge of the target environment


![image](https://github.com/user-attachments/assets/d9369071-3e3d-4ff2-acbe-41b482d02e7b)

Successful out-of-band breaches from the past have usually involved some form of supply-chain attack, third-party software compromise, malicious insiders, or unsuspecting insiders. Below are two case studies that highlight creative ways attackers reappropriate existing hardware to create impromptu wireless signals, through the methods in this lesson.


Custom Wireless over Random Access Memory (RAM)


In 2021, a security researcher discovered that memory buses on most computers emit signals that wireless-capable devices can pick up. This side-effect of signal generation allows attackers to produce covert signals, which eliminates the need for wireless hardware in the compromised air-gapped system. This is referred to as an “Air-Fi” attack. This attack relies on Double Data Rate (DDR) Synchronous Dynamic Random-Access Memory (SDRAM) buses to emit electromagnetic signals on the 2.4 GigaHertz (GHz) Wi-Fi band. It also requires custom firmware, which a malicious threat actor will have written to encode data on top of the signals. 


The attacker infects a nearby device with malware and uses that device to intercept and decode the signals. The device then transmits the signals over the internet, back to the attacker. The malware that infects the device uses electromagnetic emissions generated from the DDR SDRAM buses to exfiltrate sensitive data. The memory buses generate electromagnetic radiation at a frequency that correlates to their clock frequencies and harmonics. These emissions fall within the range of wireless frequency bands, which any device with a wireless network interface can receive. Therefore, the malware is able to trigger a series of well-timed memory operations on the infected device to modulate the stolen data into wireless signals. The data transmission rate of 100 bits per second enables attackers to use this method to exfiltrate sensitive information such as credentials, keystrokes, small files, and biometric data.


The memory operations on the emitting end do not require administrative privileges to create, which makes this technique especially dangerous in vulnerable air-gapped networks. However, creating a successful receiver on the internet side, in a close-proximity compromised device, does require kernel-level access to the wireless card and driver.


Custom Wireless over Ethernet


The Ethernet cables in a Local Area Network (LAN) serve as antennae in a novel attack known as "LANtenna". This attack enables malicious code in air-gapped computers to amass sensitive data and then encode the data over radio waves emanating from Ethernet cables. A Software-Defined Radio (SDR) receiver can then wirelessly intercept the transmitted signals near it, decode the data, and forward it to the destination the attacker uses. The malicious code can run in an ordinary user-mode process and successfully operate from within a virtual machine. 


A LANtenna attack works by using the malware in the air-gapped workstation to induce the Ethernet cable to generate electromagnetic emissions in the frequency bands of 125 MegaHertz (MHz). A nearby radio receiver then modulates and intercepts these emissions. In a proof-of-concept demo, the Ethernet cable of an air-gapped computer was able to transmit data a distance of 200 centimeters (cm) away from the receiver.


Mitigating Custom Wireless Protocols


Mitigating custom wireless protocols is difficult, as they tend to employ novel approaches to data exfiltration. However, some of these techniques employ elements for which CDAs and their teams can prepare. For example, some techniques require system-level access and precise knowledge of both the computer architecture of compromised systems, as well as the wireless network card in a close-proximity system. 


Therefore, the best practice for mitigating against novel out-of-band data exfiltration is guarding access to privileged accounts through the Principle of Least Privilege (POLP). Another effective policy measure is to prohibit the use of radio receivers in and around air-gapped networks. 


One physical countermeasure is to jam any wireless signals in and around the space by using metal shielding on cables connected to air-gapped devices. This limits electromagnetic fields emanating from the shielded wires. Additionally, physical separation of gapped and non-gapped systems mitigates the effectiveness of custom wireless signals. At least six feet of separation has also been effective for observed cases of these novel covert channels, due to their low power signals.


Bluetooth


Exfiltration over Bluetooth is possible with sufficient access and proximity. Bluetooth connections are often less secure than the primary internet-connected channel because Bluetooth is not routed through the same enterprise network. Defenders may also overlook this protocol because normal business operations do not occur over it.


Bluetooth is generally reliable up to 30 meters with most modern hardware implementations. Some developers experience communication over this protocol up to several hundred meters, with the help of custom variations to receiver sensitivity, transmit power, and antenna gain. However, hardware limitations prevent successful transfer that far, due to constraints by the systems that a threat actor is likely to compromise.


Mitigating Bluetooth Exfiltration


The simple way to mitigate the risk of Bluetooth exfiltration is to disable the Bluetooth driver from any system that has both the driver and a Bluetooth antenna. However, Bluetooth functionality may be a requirement in some systems and system operations, such as mission partner-issued mobile devices. In these cases, implement the following guidelines to mitigate an adversary’s use of Bluetooth:
Turn off Bluetooth when not explicitly using it.
Restrict device operation to the latest Bluetooth protocols.
Disable operation over Bluetooth 1.x, 2.0, or 4.0-LE (Low-Energy).

![image](https://github.com/user-attachments/assets/e522f643-e3be-47d0-910d-45747a8186ea)


--------------------------------

Rogue Devices
A rogue device is any device that is connected to the mission partner environment without the permission of the network owners. Any rogue device has the potential to wreak enormous damage or open gaping vulnerabilities by circumventing the security footprint and controls carefully emplaced, regardless of whether the device was planted maliciously or attached negligently. Some devices have the potential to cause more damage than others.

﻿

Devices that are physically placed onto the network footprint may be especially difficult to identify and weed out. They also defy comprehensive descriptions because they can exist in a variety of appearances and functions. The best way to recognize them is to become familiar with examples of the creativity with which a threat actor designs and employs these devices.

﻿

The types of devices vary widely, but may include the following:
...
The shark jack, as illustrated in Figure 10.4-2 below, appears as nothing more than a keychain ornament. However, it is actually a network auditing tool that plugs into any open Ethernet port in a mission partner’s footprint and runs preconfigured attacks or reconnaissance scripts.


![image](https://github.com/user-attachments/assets/32841179-5610-425d-9ebf-1bf85f783a0c)

LAN Tap


In an air-gapped LAN, all devices are connected using Ethernet or fiber lines. An Ethernet LAN Tap is any device that physically taps an Ethernet cable in the network, especially if it is one that is highly trafficked in the network. A LAN tap, illustrated in Figure 10.4-3 below, provides enormous insight into an organization’s operations and allows for silent sniffing of any data that passes through. Adversaries may exfiltrate data by physically retrieving the device or by configuring it to transmit all sniffed data to a nearby connected device. In the latter case, an adversary never needs to return to the tap.

![image](https://github.com/user-attachments/assets/7fee3e3c-b347-41e3-8259-1e577d2a02fe)

WiFi Pineapple®


A WiFi Pineapple is not explicitly a data exfiltration device. It is a rogue device capable of creating a wireless network. A WiFi Pineapple, illustrated in Figure 10.4-4 below, may serve as an alternative way out of a non-permissive environment, or to stand up an extremely dangerous malicious alternative. The WiFi Pineapple may have two or more antennae, depending on its model and any upgrades. The Evil Twin attack, described in an earlier lesson, used a WiFi Pineapple to lure unsuspecting team members to connect to it. The attacker achieved this by using the device to create a rogue wireless network with an identifier matching a legitimate network.

![image](https://github.com/user-attachments/assets/17eb13ea-71f8-4d7c-a530-75f72ad882ee)

Raspberry Pi


A Raspberry Pi is a custom mini-computer with extremely high versatility that serves a variety of completely innocuous purposes. However, this versatility lends itself to attacks requiring highly customized deployment as a rogue device. The commercial Information Technology (IT) support world is rife with examples of finding Raspberry Pi devices in organizational footprints, for no apparent reason. A device could be left behind to function as a printer maintainer that monitors ink and paper levels, or it could be an attacking platform that proxies into the environment behind the firewall. Below, Figure 10.4-5 illustrates the Raspberry Pi, which is frequently identifiable by its exposed circuit board and distinct logo.


![image](https://github.com/user-attachments/assets/a0a91fea-ad4a-4712-b8f3-47fa6f4e2f86)

Mitigation


To mitigate the use of rogue devices on the network, defenders should employ port security and port-based network access control (NAC). User training is also essential for urging vigilance in identifying devices that do not belong in a mission partner environment.


Port Security


Switch ports on supported switches can have configurations applied to them that restrict traffic only to known sources. This is also known as “allowlisting.” By restricting traffic to known media access control (MAC) addresses, allowlisting improves the security of a network and mitigates specific types of attacks.


Port-based NAC


The Institute of Electrical and Electronics Engineers (IEEE) 802.1x protocol, or “Dot1x,” is a standard for port-based NAC. It provides a user an endpoint authentication mechanism for wired and wireless LANs. Dot1x is generally used in enterprise and campus networks.


User Training


Relevant and affected parties, especially technical support personnel, should complete training that explains the threat of rogue devices. The training should also guide trainees on how to identify known good hardware. An organization can more easily identify anomalies if they employ recognizable and visible stickers or tagging of owned devices and train their employees to recognize these features.


![image](https://github.com/user-attachments/assets/41b30256-324c-4492-8989-0957a2346ff5)

![image](https://github.com/user-attachments/assets/000ec92a-01a8-41d2-839a-f2e5cf67af1e)


-------------------------

USB Removable Storage
Exfiltration over Removable Storage Devices
﻿

USB storage devices are useful for transferring information within an organization. However, they are a common threat vector through which data exfiltration occurs. For example, in circumstances such as an air-gapped network compromise, an adversary may exfiltrate data over a USB-connected physical device that a user has introduced to the network. The USB device could serve as the final exfiltration point or allow the adversary to hop between systems that are otherwise disconnected.

﻿

Adversaries often combine this method of exfiltration with an exploitation method that targets an active AutoRun function in vulnerable computers. A system with AutoRun enabled searches a newly inserted drive for an autorun.inf file. This file provides instructions for the system about which scripts or executables are the entry point for that drive. The system then executes those instructions without prompting the user or performing further checks. Adversaries may exploit AutoRun to conduct reconnaissance on a device, execute arbitrary commands, and script the scraping and exfiltration of sensitive data. 

﻿

Despite the threat of USB storage devices, many organizations feel that their utility outweighs the risks. In these situations, organizations should implement controls that track device usage. This allows the organizations to retain sufficient situational awareness and auditing capabilities to detect data theft incidents.
...
One of the easiest ways to mitigate USB exfiltration is to completely disallow the use of USB removable media in an environment. This may not seem palatable to an organization due to utility or efficiency concerns, but it is still worth serious consideration, in light of the risks. Organizations can accomplish this mitigation with a two-pronged approach:

Physically block the ports on each system.
Uninstall the driver for the ports.
If an organization allows USB removable media on their network, the next line of defense is to disable AutoPlay and AutoRun functions for all devices. Use the PowerShell cmdlet Set-GPRegistryValue to accomplish this across a domain. This cmdlet sets several registry keys in a group policy governing USB devices. To disable each function in PowerShell, set the registry value for the value specified, below, at the subkey listed:

AutoPlay
Registry Value: 1
Value Name: NoAutoplayfornonVolume
Subkey: HKLM\Software\Policies\Microsoft\Windows\Explorer
AutoRun (This applies to all drives and devices.)
Registry Value: FF
Value Name: Explorer!NoDriveTypeAutoRun
Subkey: HKLM\Software\Microsoft\Windows\CurrentVersion\Policies
Windows disables AutoRun by default in Windows 8 and any releases thereafter. However, creating enforced policies ensures that AutoRun is disabled in all devices and no edge cases exist.
...
Detecting data exfiltration through removable media devices is a matter of finding and extracting meaningful information from two possible types of event logs. Copying files to removable media creates events during file creation operations on the alternative drives. Threat actors who are trying to be especially subtle may create events during raw access operations in their exfiltration attempts. 

﻿

Sysmon Event ID 11, or “FileCreate”, logs when any process creates or overwrites a file. To capture this type of exfiltration using FileCreate events, the Sysmon configuration is written such that the service includes logging of any file creation events on device drives that are not the standard C:\ drive. Whenever a threat adversary attempts to write sensitive data to a USB drive, that action will be captured for further investigation.

﻿

Symon Event ID 9, or “RawAccessRead”, detects when a process attempts direct drive access using the \\.\ denotation, such as \\.\PhysicalDrive1. This technique attempts to avoid file access auditing tools, but is only available to Administrators since it requires elevated permissions. 

﻿

As a follow-up to detected device exfiltration, if logs indicate a particular staging location, further investigation could poll those locations on other domain systems as an indicator of malicious activity. For example, surveying for the presence of a folder that contains payroll data. An attacker may move payroll data to the folder C:\Windows\Temp\OldData on a compromised device. If this move is the action of a script or a re-used technique, then conducting a survey for the presence of this folder on all domain workstations can lead to other potential points of compromise.

![image](https://github.com/user-attachments/assets/17f78f5f-61cd-44a3-9515-d7221c4d2d20)

![image](https://github.com/user-attachments/assets/dc106712-8025-4c72-a4e9-d84d028f2e9c)
...
1. event.dataset:conn and destination.ip:
2. click on spike, destination.ip who was scanned, source.ip: source.port, destination.port, connection.state
3. dasdf
4. asdf
5. asdf
6. LOOK AT TIMEFRAME, control f script, xss_r=reflective, xss_s=standard (ALERT=SCRIPT)
7. LOOK AT TIMEFRAME, control f union look for union+select (SQLI=UNION)
8. PASSWORD POLICY
9. cd desktop, ls ./snmp_bruteforce.sh, if i can see both they aren't segmented, check credentials
10. eventlog
11. ssh user@172.16.3.5, sudo -l, sudo find . -exec /bin/sh \; -quit(gain access), cd /home/bob, ls, cat password.txt
12. smbmap -u Administrator -p 'Simspace1!Simspace1!' -d energy.lan -H 172.16.2.5 (example)

13. asdf
14. search uac in toolbar,
15. bcdedit /enum "{current}" 
